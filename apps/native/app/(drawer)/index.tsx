...
export default function Home() {
  ...
  if (isLoading) {
    ...
    connectionStatusText = "Connected to API";
  } else {
  ...
  return (
    <Container className="p-6">
      <View className="mb-6 py-4">
        <Text className="mb-2 font-extrabold text-4xl text-foreground">
          BETTER T STACK
        </Text>
      </View>
      ...
      {session?.user ? (
        <Card className="mb-6 p-4" variant="secondary">
          <Text className="mb-2 font-sans text-base text-foreground">
            Welcome, <Text className="font-medium">{session.user.name}</Text>
          </Text>
          <Text className="mb-4 font-sans text-muted text-sm">
            {session.user.email}
          </Text>
          <Pressable
      ...
      <Card className="p-6" variant="secondary">
        <View className="mb-4 flex-row items-center justify-between">
          <Card.Title className="text-green-400">System Status</Card.Title>
          <Chip
